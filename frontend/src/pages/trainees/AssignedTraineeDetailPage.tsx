...
export default function AssignedTraineeDetailPage() {
  ...
  const loadDetail = async () => {
    try {
      setLoading(true);
      if (!id) {
        navigate('/app/assigned-trainees');
        return;
      }
      ...
      const nextAssignment = assignmentResponse.data as AssignmentItem;
      setAssignment(nextAssignment);

      const assignmentLogs = normalizeList<AttendanceLogItem>(logResponse.data).filter(
        (log) => log.user_program_id === Number(id),
      );
      setLogs(assignmentLogs);
    } catch (error) {
      console.error('Error loading assigned trainee detail:', error);
      setAssignment(null);
      setLogs([]);
    } finally {
      setLoading(false);
  ...
  const metrics = useMemo(() => {
    ...
    return { required, completed, remaining, progress, approvedHours };
  }, [assignment?.completed_hours, assignment?.required_hours, logs]);

  if (loading) {
    return <div className="py-10 text-center text-slate-600">Loading assigned trainee...</div>;
  }
    ...
    <div className="space-y-6">
      <section className="rounded-[2rem] border border-white/70 bg-white/80 p-6 shadow-soft backdrop-blur-xl">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <p className="text-sm uppercase tracking-[0.2em] text-slate-500">Trainee Detail</p>
      ...
      <section className="grid gap-4 md:grid-cols-4">
        <MetricCard label="Required Hours" value={metrics.required.toFixed(2)} tone="ink" />
        <MetricCard label="Completed Hours" value={metrics.completed.toFixed(2)} tone="green" />
        <MetricCard label="Remaining Hours" value={metrics.remaining.toFixed(2)} tone="amber" />
        <MetricCard label="Progress" value={`${metrics.progress.toFixed(1)}%`} tone="blue" />
      </section>

      <section className="grid gap-4 md:grid-cols-3">
        <InfoCard label="Email" value={assignment.user?.email ?? 'N/A'} />
        <InfoCard
          label="Supervisor"
          value={assignment.supervisor ? `${assignment.supervisor.first_name} ${assignment.supervisor.last_name}` : 'N/A'}
        />
        <InfoCard
          label="Coordinator"
          value={assignment.coordinator ? `${assignment.coordinator.first_name} ${assignment.coordinator.last_name}` : 'N/A'}
        />
      </section>

      <section className="rounded-[2rem] border border-white/70 bg-white/80 p-6 shadow-soft backdrop-blur-xl">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <h2 className="text-lg font-semibold text-ink-900">Attendance Logs</h2>
          <div className="rounded-2xl bg-slate-100 px-4 py-2 text-sm font-semibold text-slate-600">
            Approved Hours: {metrics.approvedHours.toFixed(2)}
          </div>
        </div>
              ...
              {logs.map((log) => (
                <tr key={log.id}>
                  ...
                  <td className="px-5 py-4">
                    <span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${statusTone(log.status)}`}>
                      {log.status}
                    </span>
                  </td>
                  <td className="px-5 py-4">
                    <span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${approvalTone(log.approval_status)}`}>
                      {log.approval_status}
                    </span>
                  </td>
                </tr>
              ...
              {logs.length === 0 ? (
                <tr>
                  <td className="px-5 py-10 text-center text-slate-500" colSpan={6}>
                    No attendance logs found for this trainee.
                  </td>
                </tr>
              ) : null}
            </tbody>
