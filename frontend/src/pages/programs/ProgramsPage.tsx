      ...
      {loading ? (
        <Surface className="flex items-center justify-center py-12">
          <div className="text-center">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">Loading programs...</p>
          </div>
        </Surface>
      ) : programs.length === 0 ? (
        <EmptyState
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900">
                ...
                <th className="px-6 py-3 text-left text-sm font-semibold text-black dark:text-white">Required Hours</th>
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
              {programs.map(p => (
                <tr key={p.id} className="hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                  <td className="px-6 py-4 text-sm font-medium text-black dark:text-white">{p.name}</td>
                  <td className="px-6 py-4 text-sm text-gray-600 dark:text-gray-400">{p.required_hours} hrs</td>
