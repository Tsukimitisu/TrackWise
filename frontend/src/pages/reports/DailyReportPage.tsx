...
const DailyReportPage = () => {
  ...
  const fetchReports = async () => {
    try {
      ...
      const response = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/daily-reports`, { params });
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await axios.delete(`${import.meta.env.VITE_API_BASE_URL}/daily-reports/${id}`);
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        eyebrow="Reports"
        title="Daily Reports"
        description="Review daily work logs with a cleaner table, focused search, and consistent action placement."
        actions={
          <div className="flex flex-wrap gap-3">
            <button
              onClick={async () => {
                try {
                  await downloadDailyReportsCSV({ status: filter, search });
                } catch {
            ...
            <button
              onClick={() => navigate('/app/reports/daily/create')}
      ...
      <Surface className="p-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <TextField value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search by name, email, program, or tasks" className="lg:max-w-xl" />
      ...
      {loading ? (
        <Surface className="p-10 text-center text-slate-500 dark:text-slate-400">Loading daily reports...</Surface>
      ) : filteredReports.length === 0 ? (
        <EmptyState title="No daily reports found" description="Try a different filter or create the first entry for this workspace." />
      ) : (
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                {filteredReports.map((report) => (
                  <tr key={report.id} className="transition hover:bg-slate-50/80 dark:hover:bg-slate-900/50">
                    <td className="px-6 py-4 text-sm font-medium text-slate-900 dark:text-slate-50">{report.report_date}</td>
                    ...
                    <td className="px-6 py-4 text-sm text-slate-500 dark:text-slate-400">{report.tasks_done}</td>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex justify-end gap-2">
                        <button onClick={() => navigate(`/app/reports/daily/${report.id}`)} className="rounded-2xl bg-slate-100 px-3 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-200 dark:bg-slate-900 dark:text-slate-200 dark:hover:bg-slate-800">
                          View
                        </button>
                        {report.status === 'draft' ? (
                          <>
                            <button onClick={() => navigate(`/app/reports/daily/${report.id}/edit`)} className="rounded-2xl bg-slate-950 px-3 py-2 text-sm font-semibold text-white transition hover:-translate-y-0.5 hover:bg-slate-800 dark:bg-white dark:text-slate-900 dark:hover:bg-slate-200">
                              Edit
                            </button>
